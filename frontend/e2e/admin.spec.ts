...
test.describe('Admin Panel', () => {
  ...
  test('should display admin products list', async ({ page }) => {
    await page.route(`${API_BASE}/admin/products`, async (route) => {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ status: 'success', data: [{ _id: '1', name: 'Test Product', price: 99.99, stock: 10 }] }) });
    });
    await page.addInitScript(() => {
      localStorage.setItem('auth-storage', JSON.stringify({
        state: { user: { _id: '1', name: 'Admin User', email: 'admin@nova.com', role: 'admin' }, isAuthenticated: true, accessToken: 'test-admin-token' }
  ...
  test('should display admin orders list', async ({ page }) => {
    await page.route(`${API_BASE}/admin/orders`, async (route) => {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ status: 'success', data: [{ _id: 'o1', orderStatus: 'pending', total: 150 }] }) });
    });
    await page.addInitScript(() => {
      localStorage.setItem('auth-storage', JSON.stringify({
        state: { user: { _id: '1', name: 'Admin User', email: 'admin@nova.com', role: 'admin' }, isAuthenticated: true, accessToken: 'test-admin-token' }
  ...
  test('should display admin users list', async ({ page }) => {
    await page.route(`${API_BASE}/admin/users`, async (route) => {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ status: 'success', data: [{ _id: 'u1', name: 'User One', email: 'user1@test.com', role: 'user' }] }) });
    });
    await page.addInitScript(() => {
      localStorage.setItem('auth-storage', JSON.stringify({
        state: { user: { _id: '1', name: 'Admin User', email: 'admin@nova.com', role: 'admin' }, isAuthenticated: true, accessToken: 'test-admin-token' }
    ...
    await expect(page.getByText(/user one/i)).toBeVisible();
  });
});
